...
--Автор основного функционала: aka_zaratustra осень 2020
--Автор сервер варианта: asvdvl
local ver = "1.1.3wireless" -- версия программы
...
local function robotTryForward() -- роботы пытается сделать шаг вперед, до тех пор, пока ему это не удастся
...
local function robotMove_P0_M1()
...
local function robotMove_M1_M2()
...
local function robotMove_M2_M3()
...
local function robotMove_M3_M4()
...
local function robotMove_M4_P0()
...
local function robotMove_P0_C1()
...
local function robotMove_C1_C4()
...
local function robotMove_C4_C5()
...
local function robotMove_C5_C2()
...
local function robotMove_C2_C3()
...
local function robotMove_C3_P0()
...
local function robotGoToParkFrom_M_Crop(m) --едем на парковку. "m" - номер материнского кропа с которого мы едем
	if m == 1 then
		--начальное положение M1 смотрит вверх
		robot.turnAround()
		robotTryForward()
...
local function robotGoToPark(c) --едем на парковку. "с" - номер дочернего кропа с которого мы едем
	if c == 1 then
		--начальное положение C1 смотрит влево
		robot.turnAround()
		robotTryForward()
...
local function robotGoTo_M_Crop_FromPark(m) --едем с парковки P0 к материнскому кропу. "m" - номер кропа к которому мы едем
	if m==1 then
		--исходное положение P0 смотрит вверх
		robotTryForward()
		robotTryForward()
...
local function robotGoTo_C_Crop_FromPark(c) --едем с парковки P0 к дочернему кропу. "с" - номер кропа к которому мы едем
	if c == 1 then
...
local function grabCropsFromBarrel() --пополняем запас палок в роботе из бочки.
	...
	local cropsStackSize = 62 --размер стака палок, который робот возит с собой. 62 потому что при уничтожения кропса робот выдерает из земли и палки, и они могут попасть в нецелевой слот

	--палки лежат в первом слоте
	local returnValue = true
	local itemCount = robot.count(1)
...
local function dropToChest() --все что есть в инвентори, скидываем в сундук
...
local function destroyAllDoubleCrops() --уничтожим все двойные кропсы
...
local function howInterestingIsThisCrop(c) --оценивает полезность нового растения сравнивая статы дочернего ростения [с] со статами материнский растений . тип растения во внимание не принимается
...
--            m - превосходит материнское, где 1<m<4 - номер материнского кропа, который нужно заменить новым растением
	...
	if c_grow[c] >= grow_kill or c_resistans[c] > 0 then --если grow у дочернего растения достигло опасного значения, то это растение считаем негодным
		return 0
	end
	...
	local maxDifference = 0 --самая лучшая разница в качестве
	local m_crop_maxDifference = 0 --материнский кроп с самой лучшей разницей в качестве
	for m = 1, 4 do
		--сравниваем статы полученного растения со статами материнских растений
		local difference = (c_gain[c] + c_grow[c] - c_resistans[c]) - (m_gain[m] + m_grow[m] - m_resistans[m])
...
local function placeDoubleCrops() --ставит новые палки
...
local function findSeedsInRobotInventory() --возвращает номер слота в инвентори робота с семечками, если нет семечек, то возвращает 0, а если вообще ничего нет, то возвращает -1
...
local function analizeAndProceed(c) --функция анализа и обработки кропа. с - номер кропа.
	...
	if not analyze_result["exist"] then
		c_status[c] = "unknown"
	end
	...
	if c_cropname[c] ~= "" then -- если перед нами что-то вывелось, а не пустые палки
		...
		if c_cropname[c] ~= cropname then
			...
			if c_status[c] == "double crop" or c_status[c] == "unknown" then  --если статус растения был двойные палки, значит появилось новое растение
				...
				local interest = howInterestingIsThisCrop(c)

				--выведем инфу о новом кропе
				local interestString = ""
				if interest == 0 then
				...
				if interest == 0 then -- растение с плохими статами
					...
					local seedSlot = findSeedsInRobotInventory() --находим слот с семечками
...
if item.name ~= "IC2:itemWeedingTrowel" then --если в слоте что-то есть, но это не лопатка
	robot_error("Нет лопатки в слоте для инструмента!")
end
